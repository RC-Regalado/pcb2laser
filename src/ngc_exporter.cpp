 ...
 */
/******************************************************************************/
void NGC_Exporter::add_header(string header) { this->header.push_back(header); }
...
void NGC_Exporter::export_all(boost::program_options::variables_map &options) {
  ...
  bMetricoutput =
      options["metricoutput"].as<bool>(); // set flag for metric output
  bZchangeG53 = options["zchange-absolute"].as<bool>();
  nom6 = options["nom6"].as<bool>();

  cout << bMetricoutput << endl;
  string outputdir = options["output-dir"].as<string>();

  // set imperial/metric conversion factor for output coordinates depending on
  // metricoutput option
  cfactor = bMetricoutput ? 25.4 : 1;

  tileInfo = Tiling::generateTileInfo(options, board->get_height(),
                                      board->get_width());
  ...
  for (string layername : board->list_layers()) {
    ...
    xoffset -=
        options["x-offset"].as<Length>().asInch(bMetricinput ? 1.0 / 25.4 : 1);
    yoffset -=
        options["y-offset"].as<Length>().asInch(bMetricinput ? 1.0 / 25.4 : 1);
    if (layername == "back" ||
      ...
      if (options["mirror-yaxis"].as<bool>()) {
        ...
        yoffset -= 2 * options["mirror-axis"].as<Length>().asInch(
                           bMetricinput ? 1.0 / 25.4 : 1);
      } else {
        xoffset = -xoffset + tileInfo.boardWidth * (tileInfo.tileX - 1);
        xoffset -= 2 * options["mirror-axis"].as<Length>().asInch(
                           bMetricinput ? 1.0 / 25.4 : 1);
      }
    }
    ...
    if ((options["al-front"].as<bool>() && layername == "front") ||
      ...
      leveller.emplace(options, &ocodes, &globalVars, xoffset, yoffset,
                       tileInfo);
    }
    ...
    string of_name =
        build_filename(outputdir, options[option_name.str()].as<string>());
    ...
    cout << "DONE." << " (Height: " << board->get_height() * cfactor
         << (bMetricoutput ? "mm" : "in")
         << " Width: " << board->get_width() * cfactor
         << (bMetricoutput ? "mm" : "in") << ")";
    if (layername == "outline")
      cout << " The board should be cut from the "
           << (workSide(options, "cut") ? "FRONT" : "BACK") << " side. ";
    cout << endl;
  }
}
...
void NGC_Exporter::cutter_milling(std::ofstream &of, shared_ptr<Cutter> cutter,
                                  const linestring_type_fp &path,
                                  const vector<size_t> &bridges,
                                  const double xoffsetTot,
                                  const double yoffsetTot) {
  const unsigned int steps_num =
      cutter->stepsize == 0
          ? 1
  ...
  for (unsigned int i = 0; i < steps_num; i++) {
    ...
    for (size_t current = 1; current < path.size(); current++) {
      while (current_bridge != bridges.cend() &&
             *current_bridge < current - 1) {
        current_bridge++;
      }
      // We are now cutting to current.
      // Is this a bridge cut?
      auto is_bridge_cut =
          current_bridge != bridges.cend() && *current_bridge == current - 1;
      ...
      of << "G01 X" << (path.at(current).x() - xoffsetTot) * cfactor << " Y"
         << (path.at(current).y() - yoffsetTot) * cfactor << '\n';
    }
  }
}

void NGC_Exporter::isolation_milling(std::ofstream &of,
  ...
  for (unsigned int i = 0; i < steps_num; i++) {
    ...
    if (leveller) {
      leveller->setLastChainPoint(
          point_type_fp((path.begin()->x() - xoffsetTot) * cfactor,
                        (path.begin()->y() - yoffsetTot) * cfactor));
      of << leveller->g01Corrected(
          point_type_fp((path.begin()->x() - xoffsetTot) * cfactor,
                        (path.begin()->y() - yoffsetTot) * cfactor),
          z * cfactor);
    } else {
      // De momento aun no hay z
      // of << "G01 Z" << z * cfactor << " (leveller) \n";
    }
    of << "G01 F" << mill->feed * cfactor << "\n";
    while (iter != path.cend()) {
      if (leveller) {
        of << leveller->addChainPoint(
            point_type_fp((iter->x() - xoffsetTot) * cfactor,
...
void NGC_Exporter::export_layer(shared_ptr<Layer> layer, string of_name,
  ...
  vector<pair<coordinate_type_fp, multi_linestring_type_fp>> all_toolpaths =
      layer->get_toolpaths();
  ...
  of.setf(
      ios_base::fixed); // write floating-point values in fixed-point notation
  ...
  if (cutter) {
    for (auto &path :
         all_toolpaths[0]
             .second) { // Cutter layer can only have one tool_diameter.
  ...
  for (size_t toolpaths_index = 0; toolpaths_index < all_toolpaths.size();
       toolpaths_index++) {
    ...
    if (toolpaths_index == all_toolpaths.size() - 1) {
      tiling.setGCodeEnd(string("\nG04 P0 ( dwell for no time -- G64 should "
                                "not smooth over this point )\n") +
                         (bZchangeG53 ? "G53 " : "") + "G00 Z" +
                         str(format("%.6f") % (mill->zchange * cfactor)) +
                         " ( retract )\n\n" + postamble +
                         "M5 ( Spindle off. )\nG04 P" +
    ...
    */
    ...
    /*
     of << endl
        << (bZchangeG53 ? "G53 " : "") << "G00 Z" << mill->zchange * cfactor
        << " (Retract to tool change height)" << endl
    ...
    of << (nom6 ? "" : "M6      (Tool change.)\n")
       ...
       << " (Wait for spindle to get up to speed)" << endl;
    ...
    */
    ...
    for (unsigned int i = 0; i < tileInfo.forYNum; i++) {
      ...
      for (unsigned int j = 0; j < tileInfo.forXNum; j++) {
        double xoffsetTot = xoffset - (i % 2 ? tileInfo.forXNum - j - 1 : j) *
                                          tileInfo.boardWidth;

        if (tileInfo.enabled && tileInfo.software == Software::CUSTOM)
          of << "( Piece #" << j + 1 + i * tileInfo.forXNum << ", position ["
             << j << ";" << i << "] )\n\n";

        // contours
        for (size_t path_index = 0; path_index < toolpaths.size();
             path_index++) {
          ...
          /* if we're cutting, perhaps do it in multiple steps, but do
           * isolations just once. i know this is partially repetitive, but this
           * way it's easier to read
           */
          if (cutter) {
            cutter_milling(of, cutter, path, all_bridges[path_index],
                           xoffsetTot, yoffsetTot);
          } else {
  ...
  of << "M9 ( Coolant off. )" << endl << "M2 ( Program end. )" << endl << endl;
 ...
 */
/******************************************************************************/
void NGC_Exporter::set_preamble(string _preamble) { preamble = _preamble; }

/******************************************************************************/
/*
 */
/******************************************************************************/
void NGC_Exporter::set_postamble(string _postamble) { postamble = _postamble; }
