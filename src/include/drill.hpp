...
/*
 ...
 Also, i'm lazy, and if I turn out to be wrong splitting the code won't be much
 effort anyway.
 */
/******************************************************************************/
...
public:
  ...
  linestring_type_fp line_to_holes(const linestring_type_fp &line,
                                   double drill_diameter);
  void export_ngc(const std::string of_dir,
                  const boost::optional<std::string> &of_name,
                  std::shared_ptr<Driller> target, bool onedrill, bool nog81,
                  bool nom6, bool zchange_absolute);
  void export_ngc(const std::string of_dir,
                  const boost::optional<std::string> &of_name,
                  std::shared_ptr<Cutter> target, bool nom6,
...
private:
  ...
  std::unique_ptr<gerbv_project_t, GerbvDeleter>
  parse_project(const std::string &filename);
  std::map<int, drillbit> parse_bits();
  std::map<int, multi_linestring_type_fp> parse_holes();

  bool millhole(std::ofstream &of, double start_x, double start_y,
                double stop_x, double stop_y, std::shared_ptr<Cutter> cutter,
                double holediameter);
  double get_xvalue(double);
  double get_yvalue(double);
  std::string drill_to_string(drillbit drillbit);

  std::vector<std::pair<int, multi_linestring_type_fp>>
  optimize_holes(std::map<int, drillbit> &bits, bool onedrill,
                 const boost::optional<Length> &min_diameter,
                 const boost::optional<Length> &max_diameter);
  std::map<int, drillbit> optimize_bits();

  void
  save_svg(const std::map<int, drillbit> &bits,
  ...
  // The minimum size hole that is milldrilled.  Below this, holes are drilled
  // regularly.
